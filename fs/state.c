...
int state_init(tfs_params params) {
    ...
    pthread_rwlock_init(&open_file_lock, NULL);
...
int inode_create(inode_type i_type) {
    ...
    case T_FILE: case T_SOFTLINK:
        // In case of a new file or soft link, simply sets its size to 0
...
int add_to_open_file_table(int inumber, size_t offset) {
    pthread_rwlock_wrlock(&open_file_lock);
    for (int i = 0; i < MAX_OPEN_FILES; i++) {
        if (free_open_file_entries[i] == FREE) {
            free_open_file_entries[i] = TAKEN;
            open_file_table[i].of_inumber = inumber;
            open_file_table[i].of_offset = offset;
            pthread_rwlock_unlock(&open_file_lock);
            return i;
        }
    }
    pthread_rwlock_unlock(&open_file_lock);
...
open_file_entry_t *get_open_file_entry(int fhandle) {
    pthread_rwlock_rdlock(&open_file_lock);
